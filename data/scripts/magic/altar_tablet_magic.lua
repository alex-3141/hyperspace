...
-- hyperspace
if( GlobalsGetValue("HYPERSPACE_STATE") == "" ) then
	local matricies = EntityGetInRadiusWithTag( x, y, 128, "orthogonal_projection_matrix" )

	if (#matricies < 1) then return end

	local players = EntityGetWithTag( "player_unit" )

	if (#players < 1) then return end

	for i, matrix in ipairs(matricies) do
		local components = EntityGetComponent( matrix, "PhysicsBodyComponent" )

		if( components == nil ) then
			return
		end

		local fx, fy = EntityGetTransform( matrix )
		local distance = math.abs( x - fx ) + math.abs( y - fy )

		if ( distance < 128 ) then
			print("Hyperspace activated")
			GamePrintImportant( "Matrix Detected", "" )
			GlobalsSetValue("HYPERSPACE_STATE", "SPIN_UP" )

			function removeComponent(componentID)
				local comps = EntityGetComponent( matrix, componentID )
				if( comps ~= nil ) then
					for i, comp in ipairs(comps) do
						EntityRemoveComponent( matrix, comp )
					end
				end
			end

			removeComponent('PhysicsBodyComponent')
			removeComponent('PhysicsImageShapeComponent')
			removeComponent('ItemComponent')
			removeComponent('PhysicsThrowableComponent')
			removeComponent('VelocityComponent')
			removeComponent('SpriteComponent')
			removeComponent('AbilityComponent')
			removeComponent('AudioLoopComponent')
			removeComponent('AudioLoopComponent')
